...
const props = defineProps({
  modelValue: {
    type: Array, // Expects an array of tag IDs: [1, 5, 10] or temp IDs like ['new_my-tag_timestamp']
    default: () => [],
  },
  label: {
    type: String,
...
// Changed from const to function declaration for hoisting
function updateModelValue() {
  // Ensure that modelValue always contains IDs (numeric or temporary string IDs)
...
watch(() => props.initialTags, (newTags) => {
  ...
  if (newTags && newTags.length > 0) {
    // Filter out any potential duplicates from the newTags array itself (though API should provide unique)
    const uniqueNewTags = [];
