...
echo "Starting test for task notes and status changes...\n";
...
if (! $milestone) {
    ...
    $milestone = $project->milestones()->create([
        'name' => 'Test Milestone',
        'description' => 'Test milestone for task notes and status changes',
        'status' => 'Not Started',
    ]);
}
...
$task = new Task;
$task->name = 'Test Task for Notes and Status Changes';
$task->description = 'This is a test task to verify notes and status changes';
$task->status = 'To Do';
...
// Test 1: Add a note to the task
echo "\n--- Test 1: Adding a note to the task ---\n";
try {
    $result = $task->addNote('This is a test note for the task', $user);
...
try {
    ...
    $task->refresh();
    if ($task->status === 'Done') {
        echo "✅ Task status is correctly set to 'Done'\n";
    } else {
        echo "❌ Task status is NOT set to 'Done', current status: {$task->status}\n";
    }

    // Verify that the actual_completion_date is set
    if ($task->actual_completion_date) {
        echo "✅ Task actual_completion_date is set to: {$task->actual_completion_date}\n";
    } else {
        echo "❌ Task actual_completion_date is NOT set\n";
    }
} catch (\Exception $e) {
    echo 'Error completing task: '.$e->getMessage()."\n";
}

// Check the logs for Google Chat messages
echo "\n--- Checking logs for Google Chat messages ---\n";
echo "Please check the Laravel logs for Google Chat message entries.\n";
echo "You should see log entries for:\n";
echo "1. The initial task creation message\n";
echo "2. The task note message\n";
echo "3. The task started message\n";
echo "4. The task completed message\n";
