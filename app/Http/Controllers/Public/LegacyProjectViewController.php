...
/**
 * The classic (pre-redesign) Vue project page, live alongside the React portal.
 *
 * This is the controller as it stood before the redesign, restored verbatim except for
 * the share-code lookup below. It serves `Pages/Public/ProjectView.vue` at
 * /projects/classic/{code} with its own endpoints and its own localStorage session, so
 * anyone who hits trouble on the new portal has somewhere to go. The portal project page
 * links here; see PortalProjectPresenter's `classic_url`.
 *
 * It shares nothing with the portal but OtpService and the models, so portal changes
 * cannot break it — which is the entire reason it is worth keeping around.
 *
 * Everything here is deliberately frozen. Fixes belong in PublicProjectController and
 * Portal\PortalController. Known behaviour kept on purpose: proposals are upserted
 * one-per-PROJECT, so quoting a second phase overwrites the first, and the full
 * 64-character share token is exposed in the page props because the page uses it as its
 * API path and localStorage key.
 *
 * Retire by setting PORTAL_CLASSIC=false (config/portal.php); the link and these routes
 * disappear together. Then delete this file and the Vue page.
 *
 * @see routes/web.php — the config('portal.classic') group
 */
class LegacyProjectViewController extends Controller
{
    /**
     * Same floor as PublicProjectController::MIN_SHARE_CODE_LENGTH.
     */
    ...
    /**
     * Entry point for /projects/classic/{code}.
     *
     * Takes either the 12-character share code or the full token, because the portal
     * page that links here only ever holds the code. The page itself still receives the
     * full token in its props — every one of its endpoints is keyed on it.
     */
    public function showClassic(string $code): Response
    {
    ...
    /**
     * NOT restored verbatim, and must not be.
     *
     * The original ran `like $code.'%'` with no minimum length and no wildcard escaping,
     * so `GET /projects/public/x/%25` matched the first enabled project and returned its
     * full share token in the page props — character-by-character probing recovered any
     * project's token. Turning the legacy flag on must not reopen that.
     */
