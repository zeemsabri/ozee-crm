...
// --- REVISED: Top-Level Slide Layout Renderer ---
// Added a new case for a special Cover Slide layout.
function renderSlideLayout(slide, index) {
    const allBlocks = slide.content_blocks || [];

    // --- NEW: Special handling for CoverSlide template ---
...
// It now accepts an optional 'context' to render blocks differently based on the layout.
function renderBlock(block, context = 'default') {
    ...
    switch (block.block_type) {
        ...
        case 'image':
            // --- NEW: Context-aware rendering for images ---
            if (context === 'cover') {
