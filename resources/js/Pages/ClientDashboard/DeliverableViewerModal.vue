...
watch(() => props.deliverable, (newVal) => {
    if (newVal) {
        ...
        // Only mark as read if deliverable is properly loaded
        if (props.isOpen) {
...
// Add a separate watch for isOpen to handle marking as read when modal opens
...
const markDeliverableAsRead = async () => {
    if (props.deliverable && (!props.deliverable.client_interaction || !props.deliverable.client_interaction.read_at)) {
        try {
            ...
            // Don't close modal on this error
        }
    }
};
