...
import JsonViewerV2 from './JsonViewerV2.vue';

const store = useAutomationsV2Store();
const isOpen = computed(() => store.showLogs);

const close = () => store.hideLogs();
const meta = computed(() => store.logsMeta);
...
const goPrev = () => {
  if (canPrev.value) store.fetchLogs(store.logsWorkflowId, { page: meta.value.current_page - 1 });
};
const goNext = () => {
  if (canNext.value) store.fetchLogs(store.logsWorkflowId, { page: meta.value.current_page + 1 });
};
  ...
  template: `
    <div class="space-y-1">
      <div 
        ...
        class="group flex items-center gap-3 p-2.5 rounded-xl cursor-pointer transition select-none"
        :class="[
      ...
      <!-- Nested Children -->
      <div v-if="step.children?.length" class="ml-5 border-l-2 border-slate-100 pl-1.5 space-y-1">
        <StepItem 
...
<template>
  <div v-if="isOpen" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-[95vw] h-[90vh] flex flex-col overflow-hidden border border-slate-200">
      <!-- Header -->
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm font-medium text-slate-500">
              Tracing logic for Automation ID: <span class="text-indigo-600">{{ store.logsWorkflowId }}</span>
            </p>
          </div>
        </div>
        <div class="flex items-center gap-4">
            <div class="flex items-center bg-white border border-slate-200 rounded-xl px-2 py-1 space-x-2 shadow-sm">
                <button @click="goPrev" :disabled="!canPrev" class="p-1.5 hover:bg-slate-100 disabled:opacity-30 rounded transition">
                ...
                <button @click="goNext" :disabled="!canNext" class="p-1.5 hover:bg-slate-100 disabled:opacity-30 rounded transition">
      ...
      <div class="flex-1 overflow-hidden relative">
        <div v-if="store.isLoadingLogs" class="absolute inset-0 z-10 bg-white/80 flex items-center justify-center backdrop-blur-sm">
            <div class="flex flex-col items-center gap-3">
                <div class="w-10 h-10 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin"></div>
                <p class="text-sm font-bold text-slate-500 uppercase tracking-widest">Fetching logs...</p>
            </div>
        </div>

        <splitpanes class="default-theme h-full">
          <!-- Left Pane: Timeline -->
          <pane min-size="25" size="30" class="bg-slate-50 border-r border-slate-200 overflow-y-auto">
            <div class="p-4 space-y-4">
              <div v-if="!groupedExecutions.length && !store.isLoadingLogs" class="text-center py-20">
                <ClockIcon class="w-12 h-12 text-slate-200 mx-auto mb-3" />
                <p class="text-sm font-bold text-slate-400">No execution logs found yet.</p>
              </div>

              <div v-for="exec in groupedExecutions" :key="exec.id" 
                class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm transition hover:shadow-md"
              ...
              >
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-[10px] uppercase font-semibold text-slate-400 flex items-center gap-1 mt-0.5">
                  ...
                  <div class="flex items-center gap-2">
                    <span class="text-[10px] uppercase tracking-wider font-bold text-slate-400">{{ exec.steps.length }} steps</span>
                    <ChevronDownIcon 
                ...
                <!-- Steps List -->
                <div v-if="expandedExecutions.has(exec.id)" class="bg-slate-50/50 p-3 space-y-1">
                  <StepItem 
          ...
          <pane class="bg-slate-50 flex flex-col overflow-hidden">
            <div v-if="!selectedStep" class="flex-1 flex flex-col items-center justify-center p-10 text-center opacity-40">
                ...
                <h4 class="text-lg font-bold text-slate-500">Select a step to investigate</h4>
                <p class="text-sm font-medium text-slate-400">Execution details, inputs, and outputs will appear here.</p>
            </div>

            <template v-else>
                <!-- Detail Header -->
                <div class="px-6 py-5 border-b flex items-start justify-between bg-white z-20">
                    <div>
                        ...
                        <div class="flex items-center gap-4 text-xs font-bold uppercase tracking-wider text-slate-400">
                             <span class="flex items-center gap-1.5"><ClockIcon class="w-3.5 h-3.5" /> {{ formatDate(selectedStep.executed_at) }}</span>
                             <span v-if="selectedStep.duration_ms" class="px-2 border-l border-slate-200">{{ selectedStep.duration_ms }}ms</span>
                             <span v-if="selectedStep.token_usage" class="px-2 border-l border-slate-200">{{ selectedStep.token_usage }} tokens</span>
                        </div>
                    </div>
                </div>

                <!-- Tabs Navigation -->
                <div class="flex items-center px-6 border-b bg-white">
                    <button 
                        v-for="tab in ['output', 'input', 'config', 'error']" 
                        :key="tab"
                        @click="activeTab = tab"
                        class="px-4 py-3 text-[11px] font-bold uppercase tracking-widest transition-all border-b-2 -mb-px"
                        :class="[
                            activeTab === tab 
                ...
                <div class="flex-1 overflow-auto p-0 bg-slate-50">
                    <!-- Output Tab -->
                    <div v-show="activeTab === 'output'" class="animate-in fade-in slide-in-from-bottom-2 duration-300 h-full flex flex-col p-6 space-y-6">
                        <JsonViewerV2 
                            title="Parsed Output" 
                            :data="selectedStep.parsed_output" 
                        />
                        <JsonViewerV2 
                            title="Raw Data" 
                            :data="selectedStep.raw_output" 
                            :startExpanded="false"
                        />
                    </div>

                    <!-- Input Tab -->
                    <div v-show="activeTab === 'input'" class="animate-in fade-in slide-in-from-bottom-2 duration-300 h-full p-6">
                        <JsonViewerV2 
                            title="Input Context (Variables)" 
                            :data="selectedStep.input_context" 
                        />
                    </div>

                    <!-- Config Tab -->
                    <div v-show="activeTab === 'config'" class="animate-in fade-in slide-in-from-bottom-2 duration-300 h-full p-6 space-y-6">
                        <JsonViewerV2 
                            title="Step Configuration" 
                            :data="selectedStep.step?.step_config" 
                        />
                        <JsonViewerV2 
                            v-if="selectedStep.step?.condition_rules"
                            title="Logic Rules" 
                            :data="selectedStep.step.condition_rules" 
                        />
                    </div>

                    <!-- Error Tab -->
                    <div v-show="activeTab === 'error'" class="animate-in fade-in slide-in-from-bottom-2 duration-300 h-full p-6">
                        <div v-if="selectedStep.error_message" class="bg-red-50 border border-red-200 rounded-xl p-6 shadow-sm">
                            <div class="flex items-center gap-2 mb-3 text-red-600">
                                <AlertCircleIcon class="w-5 h-5" />
                                <h5 class="font-bold">Execution Error</h5>
                            </div>
                            <p class="text-sm text-red-800 font-mono leading-relaxed bg-white/70 p-4 rounded-lg border border-red-100 break-words">
                        ...
                        <div v-else class="flex flex-col items-center justify-center py-20 opacity-30">
                            ...
                            <p class="text-sm font-bold uppercase tracking-widest text-slate-500">No errors reported for this step</p>
                        </div>
                    </div>
                </div>
            </template>
