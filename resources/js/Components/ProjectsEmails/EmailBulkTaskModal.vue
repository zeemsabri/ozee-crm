...
const loadUsers = async () => {
  ...
  try {
    const { data } = await axios.get(`/api/projects/${props.projectId}/sections/users`, { params: { type: 'users' } });
    // API may return { users: [...] } or a mixed payload; try to extract users list
...
// Format data for API with per-task assignee
const formatDataForApi = (data) => ({ tasks: data.map(t => ({ ...t, assigned_to_user_id: t.assigned_to_user_id || null })) });
...
const handleSubmit = (close) => {
  messageBox.value = { show: true, text: 'Tasks submitted successfully!', type: 'success' };
  setTimeout(() => { messageBox.value.show = false; emit('tasks-submitted'); close(); }, 1200);
};
</script>
...
<template>
  <BaseFormModal
    ...
    :api-endpoint="`/api/emails/${emailId}/tasks/bulk`"
    :http-method="'post'"
    :form-data="formData"
  ...
  >
    ...
    <template #footer="{ close }">
      <div class="flex justify-end space-x-3">
        <SecondaryButton @click="close" type="button">Cancel</SecondaryButton>
        <PrimaryButton type="submit" :disabled="isSubmitDisabled" :class="{ 'opacity-50 cursor-not-allowed': isSubmitDisabled }">Create Tasks</PrimaryButton>
      </div>
    </template>
  </BaseFormModal>
</template>
