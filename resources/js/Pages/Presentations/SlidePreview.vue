<template>
    <div class="border border-gray-200 rounded-lg p-6 bg-gray-100 relative overflow-y-auto h-full transition-all duration-300">
        ...
        <div class="absolute bottom-4 right-4 flex gap-2">
            <button @click="toggleTheme" class="btn btn-xs" aria-label="Toggle theme">
                {{ previewTheme === 'light' ? 'Dark' : 'Light' }} Mode
            </button>
            <zoom-controls v-model="zoomLevel" aria-label="Zoom controls" />
        </div>
    </div>
</template>
...
function getRenderer(b) {
    ...
    if (b.block_type === 'paragraph') {
        return {
            render() {
                return h('p', { class: `text-gray-700 ${classes}` }, c.text || '');
            },
        };
    }
